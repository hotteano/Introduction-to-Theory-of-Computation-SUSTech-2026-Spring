# CS338 Introduction to Theory of Computation 详细复习 Note

资料覆盖：`ppts/` 中全部课件，`Week1` 至 `Week13` 的作业题面与 LaTeX/PDF 答案，`answers/` 中 Assignment 1-13 官方答案。  
...
### 0.1 Construction proof 必须写完整构造
...
- 并：新起始态 \(q_0\)，从 \(q_0\) 用 \(\epsilon\)-transition 到两个旧起始态；接受态为两个 NFA 的接受态并集。
...
- “不含某 substring”：先构造“含该 substring”的 DFA，再补集；或直接设置陷阱态。
...
- contains at least four `1`s
- contains substring `0101`
- length at least 4 and fourth symbol is `1`
- starts with `1` and has odd length, or starts with `0` and has even length
- contains at least two `0`s and at most one `1`
- empty language
- all strings except \(\epsilon\)

补集构造题：

- 不含 `ba`：先构造含 `ba` 的 DFA，再翻转接受态。
- 既不含 `ab` 也不含 `ba`：等价于 \(a^*\cup b^*\)，也可先构造含 `ab` 或 `ba` 再补。
- 不在 \(b^*a^*\) 中：等价于含 substring `ab`，注意方向不要写反。旧答案中题面与文字有编码混乱，复习时按答案源码的构造理解。

交集构造题：

- 至少两个 `a` 且至少三个 `b`：状态 \((i,j)\)，其中 \(i\in\{0,1,\ge2\}\)，\(j\in\{0,1,2,\ge3\}\)，接受 \((\ge2,\ge3)\)。
- 偶数个 `a` 且一个或两个 `b`：状态记录 `a` parity 和 `b` count \(0,1,2,\ge3\)，接受 `(even,1)` 与 `(even,2)`。
- 以 `a` 开头且至多两个 `b`：可以先判断首字符，再计数 `b`。
...
### 2.3 NFA 转 DFA：subset construction 完整写法
...
1. 起始态：
   ...
   q_D=E(\{q_0\}).
   \]
2. 对每个已经发现的子集状态 \(S\subseteq Q\) 和每个输入符号 \(a\in\Sigma\)，计算
   \[
...
### 2.4 NFA 转 DFA 示例 1：无 \(\epsilon\) 的 NFA

构造一个识别“以 `01` 结尾”的 NFA：
\[
...
\delta(q_1,1)=\{q_2\},
\]
其它转移为空集。直觉是：\(q_0\) 任意循环，看到某个 `0` 时非确定性猜它是倒数第二个字符。
...
### 2.5 NFA 转 DFA 示例 2：带 \(\epsilon\)-closure
...
\]
\(B\) 和 \(C\) 都接受，因为它们分别包含 \(y\) 或 \(u\)。这样得到的 DFA 精确接受 `a` 和 `ab`。

### 2.6 正则表达式

正则表达式递归定义：

- \(\emptyset,\epsilon,a\) 是正则表达式。
- 若 \(R_1,R_2\) 是正则表达式，则 \(R_1\cup R_2\)、\(R_1R_2\)、\(R_1^*\) 是正则表达式。
...
- \(\{\epsilon\}\)：单个起始接受态，无其它转移。
- \(0^*\)：单个起始接受态，`0` 自环。

Closure construction 题必须按证明构造：

- Union：新起点 \(\epsilon\) 到两个起点。
- Concatenation：第一个接受态 \(\epsilon\) 到第二个起点。
- Star：新起点接受，并从旧接受态 \(\epsilon\) 回旧起点。
- Single accept state：新增 \(q_a\)，旧接受态 \(\epsilon\) 到 \(q_a\)。

Subset construction 题要列子集状态表。答案源码里示例：

- 无 \(\epsilon\) 的 NFA：状态集形如 \(\emptyset,\{1\},\{2\},\{1,2\}\)。
- 有 \(\epsilon\) 的 NFA：起始态是 \(\epsilon\)-closure，例如 \(\{1,2\}\)，接受态为包含旧接受态的子集。

Bonus：语言“不含一对相隔奇数个符号的 1”。答案思路是先构造补语言 NFA：猜第一个 `1`，用两个状态记录中间间隔奇偶，若在奇数间隔后看到第二个 `1` 则接受；再取补得到原语言 DFA。
...
- 对两态 automaton，先加 \(s\) 和 \(f\)，若 1 到 2 为 \(b\)，2 自环 \(a\)，2 回 1 为 \(b\)，1 自环 \(a\)，最终答案形如
  \[
  (a\cup ba^*b)^*ba^*.
  \]

注释语言 \(C\)：以 `/#` 开始，以 `#/` 结束，中间不能提前出现 `#/`。答案给的正则式可简化记为：
\[
/\#(\#^*(a\cup b)\cup /)^*\#^+/
\]
...
- palindrome：非确定性猜中点，前半段压栈，后半段逐字符匹配弹栈。
- 奇数长度中间为 0：先压前半段计数，读中间 `0`，后半段每读一个符号弹一个计数符号。
...
- 只有一个接受态 \(q_{\mathrm{acc}}\)。
- 接受前栈必须为空。
- 每个转移只做一种栈操作：要么 push 一个栈符号，要么 pop 一个栈符号。若原转移替换多个符号，用中间状态拆开。
...
### 4.8 PDA 转 CFG 示例：从 PDA 得到 \(S\to0S1\mid\epsilon\)
...
\]
含义：在 \(p\) 中每读一个 `0` 就 push 一个 \(X\)；用 push \(Y\)、pop \(Y\) 的配对完成从 push 阶段到 pop 阶段的切换；在 \(s\) 中每读一个 `1` 就 pop 一个 \(X\)。
...
**2-PDA 识别 \(\{a^nb^nc^n\}\)**  
两个栈即可模拟三段相等：读 `a` 时压 stack1；读 `b` 时压 stack2；读 `c` 时同时弹两个栈；输入结束且两栈空则接受。这说明 2-PDA 比 1-PDA 强。

**Left-reset TM 模拟普通左移**  
普通 TM 想从当前格左移时，left-reset machine 用标记法：

1. 标记当前格，reset 到最左端。
2. 从左端开始逐步找当前格前一格。
3. 用额外标记推进“候选前驱”。
4. 当候选右边是原当前格，擦标记并停在前驱格。
...
- \(\#0=\#1\)：反复找一个未标记 0 和一个未标记 1 配对标记；若最后只剩标记则接受。
- \(\#0=2\#1\)：每找一个 `1`，匹配两个 `0`。
...
1. 将 \(R\) 转为 NFA。
2. 将 NFA 转为 DFA \(M'\)。
3. 运行 \(EQ_{DFA}\) 于 \(\langle M,M'\rangle\)。
...
- \(L(M)\) infinite：非平凡语言性质，undecidable。
- \(1011\in L(M)\)：非平凡语言性质，undecidable。
- \(L(M)=\Sigma^*\)：非平凡语言性质，undecidable。

### 6.6 Assignment 9-10 对齐
...
- \(\overline{SAT}\in P\)：严格说是开放问题；若它在 P，则 \(P=NP\)。作业官方 revised answer 更严谨地说明 unknown；旧简答里写 False 是基于通常相信 \(P\ne NP\)，考试应按题目/答案口径说明“unknown unless assuming \(P\ne NP\)”。
...
3. 设置 \(k=m\)。Assignment 12 Q1 中 \(m=3\)，所以 \(k=3\)。

正确性：

- 若 \(\phi\) satisfiable，从每个 clause 选一个 true literal。这些 literal 互不矛盾，所以对应顶点两两相连，形成 \(m\)-clique。
...
- 满足赋值 -> 按变量真值选择每个 gadget 的遍历方向，对每个 clause 通过一个 true literal 的 wire 插入 clause vertex，得到 Hamiltonian path。
- Hamiltonian path -> 每个变量 gadget 的方向给出赋值；每个 clause vertex 被访问必须通过某个 literal wire，该 literal 在对应方向下为 true，所以每个 clause satisfied。

### 9.6 Directed HAMPATH -> Undirected HAMPATH
...
- 满足赋值 -> 每个变量 gadget 选 true literal 顶点；每个 clause 三角形中留下一个 true literal 顶点不选，选另两个。变量边、三角形边、连接边都被覆盖，大小 \(v+2c\)。
- Vertex cover 大小 \(\le v+2c\) -> 每个变量边至少选一个，共至少 \(v\)；每个三角形至少选两个，共至少 \(2c\)。因此必须恰好选这些数量。每个 clause 三角形有一个未选顶点，其连接边必须由变量 gadget 中同名顶点覆盖，令该 literal 为 true，即每个 clause 有 true literal。

### 9.8 3COLOR NP-complete
...
2. 对任意 \(A\in NP\)，取多项式时间 NTM \(M\) 决定 \(A\)。给输入 \(w\)，构造公式 \(\phi_{M,w}\)，使
   \[
...
| Assignment 10 | \(EQ_{CFG}\) co-recognizable、mapping reduction、Rice、CYK、P 闭包、CONNECTED/ALL_DFA/EQ_DFA in P |
...
1. 自动机题：先写“状态记录什么”，再写状态/转移/接受态。
2. 构造证明题：必须给完整 tuple 或完整规则，不要只说“按闭包性”。
3. Pumping 题：选择的串必须在语言中，且分解必须任意。
4. 归约题：方向绝对不能反。从已知难问题归约到目标问题。
5. NP-complete 题：先 in NP，再 NP-hard。
6. Rice 题：先确认是 \(L(M)\) 的非平凡语义性质，不是机器编码的语法性质。
7. P/NP 题：注意“可验证”与“可求解”不同；HAMPATH 在 NP，但不知道是否在 P。
