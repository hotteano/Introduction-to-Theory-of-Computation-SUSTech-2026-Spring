...
\begin{tikzpicture}[->,>=stealth,shorten >=1pt,auto,node distance=2.5cm, semithick]
  ...
  \node[state,accepting, initial] (q3) [below of=q2] {$q_3$};
...
\begin{center}
    \begin{tikzpicture}[->,>=stealth,shorten >=1pt,auto,node distance=2.5cm, semithick]
      ...
      \node [state] (q6) [above of=q4] {$q6$};

      \path (q1) edge node {0, 1} (q2)
            (q2) edge node {0, 1} (q3)
            (q3) edge node {0, 1} (q4)
            (q4) edge node {1} (q5)
            (q4) edge [bend right] node {0} (q6)
...
\begin{center}
   \begin{tikzpicture}[->,>=stealth,shorten >=1pt,auto,node distance=2.5cm, semithick]
   % 第一行：0个1的情况
   \node[state,initial] (q00) {$q_{0,0}$}; 
   \node[state] (q10) [right of=q00] {$q_{1,0}$};
   \node[state,accepting] (q20) [right of=q10] {$q_{2,0}$};
   
   % 第二行：1个1的情况
   \node[state] (q01) [below of=q00] {$q_{0,1}$};
   \node[state] (q11) [right of=q01] {$q_{1,1}$};
   \node[state,accepting] (q21) [right of=q11] {$q_{2,1}$};
   
   % 死亡状态：2个1的情况
   \node[state] (qt) [below of=q11] {$q_{trap}$};

   \path[->] 
    % 0 的转移（水平）
    (q00) edge node {0} (q10)
    (q10) edge node {0} (q20)
    (q20) edge [loop right] node {0} (q20)
    (q01) edge node {0} (q11)
    (q11) edge node {0} (q21)
    (q21) edge [loop right] node {0} (q21)
...
\begin{center}
   ...
   % 符合条件的“主干道”
   \node[state,accepting] (ok0) [right of=start] {$q_{ok,0}$};
   \node[state,accepting] (ok1) [right of=ok0] {$q_{ok,1}$};
   \node[state,accepting] (ok2) [right of=ok1] {$q_{ok,2}$};
   
   % 死胡同（不以a开头，或者b超过2个）
   \node[state] (trap) [below of=ok1] {$q_{trap}$};

   \path[->] 
    % 开头判断
    (start) edge node {a} (ok0)
    (start) edge node {b} (trap)
    
    % 处理 a (自环，因为不增加 b 的计数)
    (ok0) edge [loop above] node {a} (ok0)
    (ok1) edge [loop above] node {a} (ok1)
    (ok2) edge [loop above] node {a} (ok2)
    
    % 处理 b (计数增加)
    (ok0) edge node {b} (ok1)
    (ok1) edge node {b} (ok2)
    (ok2) edge node {b} (trap)
    
    % 死胡同循环
    (trap) edge [loop below] node {a,b} (trap);
\end{tikzpicture}
\end{center}
