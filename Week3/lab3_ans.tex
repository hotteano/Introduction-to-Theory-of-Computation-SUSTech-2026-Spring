...
We convert the given finite automata to regular expressions using the state elimination method.

\subsection{Part (a)}

\paragraph{Step 1: Original NFA}
...
\paragraph{Step 2: Add New Start and Accept States}
...
\paragraph{Step 3: Eliminate State 2}
...
\paragraph{Step 4: Eliminate State 1}
...
For the DFA in Question 3(b), states 1 and 3 are accepting.

\paragraph{Step 1: Add New Start and Accept States}

Add new start state $s$ and new accept state $f$:
...
\paragraph{Step 2: Eliminate State 2}
...
\paragraph{Step 3: Eliminate State 3}
...
\textbf{Final Answer:}
\[
\boxed{\left((a \cup b)a^*b(ba^*b)^*a\right)^*\left(\epsilon \cup (a \cup b)a^*b(ba^*b)^*\right)}
\]
